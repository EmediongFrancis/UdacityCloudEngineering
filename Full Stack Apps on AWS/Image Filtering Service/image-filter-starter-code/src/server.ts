...
(async () => {
  ...
  app.get("/filteredimage", async (req: Request, res: Response) => {
    ...
    const { image_url }: { image_url: string } = req.query as any;
    ...
    // Validate the image_url query with a regular expression.
    const expression: RegExp = /(http(s?):)([/|.|\w|\s|-])*\.(?:jpg|gif|png|bmp|jpeg|tiff)/g;
    const regex: RegExp = new RegExp(expression);

    // If the image_url query is not valid, return a 400 status code.
    if (!image_url.match(regex)) {
      res.status(400).send({ message: "Image URL is not valid." });
      return;
    }
    ...
    if (!image_url.toLowerCase().endsWith(".jpeg")
      ...
      res.status(400).send({ message: "Image format not supported. Use jpeg, jpg, png, bmp, or tiff." });
      return;
    }
